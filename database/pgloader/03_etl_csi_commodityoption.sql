...
DO $$ 
DECLARE 
 expected_rows     integer := 329; -- 1 ignored because of MXP currency
 actual_rows       integer;
 source_id         integer;
 class_id          integer;
BEGIN 
    ...
    SELECT id INTO STRICT source_id FROM instrument_source 
        WHERE name = 'CSI/COMMODITYOPTION';

    -- TODO: check this one
    SELECT id INTO STRICT class_id FROM instrument_class 
        WHERE name = 'CommodityOption';
    ...
    INSERT INTO instrument (
        ...
        current_timestamp, 'ETL' 
    FROM etl.etl_csi_commodityoption AS etl 
    ;
